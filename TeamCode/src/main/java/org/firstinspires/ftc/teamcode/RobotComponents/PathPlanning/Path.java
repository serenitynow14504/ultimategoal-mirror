...
public class Path {
    ...
    public double[] getXs() {
        return Util.extractXs(points);
    }

    public double[] getYs() {
        return Util.extractYs(points);
    }


    public void calculateClosestData(VectorD point) {
        currentRobotPos = point;
        Pair<VectorD, Double> closest = new Pair<>(segments[0].getPointA(), Util.distance(point, segments[0].getPointA()));
