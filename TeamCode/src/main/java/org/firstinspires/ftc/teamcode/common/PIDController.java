...
public class PIDController
{
    ...
    private double m_absolute_tolerance;
    private double m_setpoint = 0.0;
    private double m_error = 0.0;
    private double m_result = 0.0;
    private double m_originalInput = 0;
    private boolean m_useWindowing = false;
    ...
    private void calculate()
    {
        ...
        if (m_enabled)
        {
            ...
            if(m_useWindowing) {
                m_totalError -= m_windowBuffer[(windowCounter+1)%m_windowBuffer.length];
                m_windowBuffer[(windowCounter+1)%m_windowBuffer.length] = m_error;
                windowCounter++;
            }
    ...
    public double getProgress() {return (m_input - m_originalInput)/(m_setpoint - m_originalInput);}
    ...
    {
        //return (Math.abs(m_error) < Math.abs(m_tolerance / 100.0 * (m_setpoint -
        // m_originalInput)));
        //return (Math.abs(m_error) < Math.abs(m_tolerance / 100.0 * (m_maximumInput -
        // m_minimumInput)));
        return Math.abs(m_error) < m_absolute_tolerance;
    }

    public void setOriginalInput(double esjio) {
        m_originalInput = esjio;
    }
    ...
    public void setWindowing(int windowSize) {
        m_useWindowing = true;
        m_windowBuffer = new double[windowSize];
        for (int i=0; i<windowSize; i++) m_windowBuffer[i] = 0.0;
    }
}